-- Backend hardening: complete account deletion + forum report dedupe.
...
create unique index if not exists forum_reports_post_reporter_idx
...
begin
  ...
  delete from public.forum_reports where reporter_id = p_clerk_user_id;
  ...
  set
    ...
    actor_name = 'Raderad användare'
  where actor_id = p_clerk_user_id;
  ...
  delete from public.app_store_entitlements where clerk_user_id = p_clerk_user_id;
